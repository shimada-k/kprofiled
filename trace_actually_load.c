...
#define DEVICE_FILE			"/dev/trace_actually_load"
#define FLUSH_PERIOD			2000	/* デバイス側でCLISTに対してポーリングする周期（ミリ秒で指定） */
#define CLIST_NR_NODE		6	/* CLISTでのノード数 */
...
/*
	リソースを確保する関数
*/
static int trace_actually_load_alloc_resources(void)
{
	char data_path[STR_PATH_MAX];

	snprintf(data_path, STR_PATH_MAX, "%s%s", wd_path, "trace_actually_load.data");
...
/*
	...
	return 成功:1 失敗:0
*/
static int trace_actually_load_free_resources(const char *called)
{
	NOTICE_MESSAGE("%s calls trace_actually_load_free_resources()\n", called);
...
/*
	...
	返り値　成功:1 失敗:0
*/
static int trace_actually_load_init(void)
{
	struct ioc_submit_spec submit_spec;
	//struct sigaction act;

	if(trace_actually_load_alloc_resources() == 0){
		ERR_MESSAGE("%s failed", log_err_prefix(trace_actually_load_alloc_resources));
		return 0;
	}

	/* デバイスの準備 */
	submit_spec.pid = (int)getpid();
	submit_spec.signo = SIGUSR2;
	...
	if(ioctl(dev, IOC_SUBMIT_SPEC, &submit_spec) < 0){
		ERR_MESSAGE("%s IOC_SUBMIT_SPEC", log_err_prefix(trace_actually_load_init));
		return 0;
	}
...
*/
static void trace_actually_load_final(void *arg)
{
	int nr_wcurr, grain;
	ssize_t size;
	...
	if(ioctl(dev, IOC_USEREND_NOTIFY, &nr_wcurr) < 0){
		ERR_MESSAGE("%s IOC_USEREND_NOTIFY", log_err_prefix(trace_actually_load_final));
		trace_actually_load_free_resources(__func__);
	}
	...
	if(nr_wcurr >= READ_NR_OBJECT){
		...
		if(buffer == NULL){
			ERR_MESSAGE("%s buffer is NULL", log_err_prefix(trace_actually_load_final));
		}
	...
	if(trace_actually_load_free_resources(__func__) == 0){
		exit(EXIT_FAILURE);
	}
}
...
{
	if(trace_actually_load_init() == 0){
		ERR_MESSAGE("%s failed", log_err_prefix(trace_actually_load_init));
	}

	pthread_cleanup_push(trace_actually_load_final, NULL);

	while(1){
		sleep(1);
		pthread_testcancel();
		/* もしSIGUSR1を受信したら, trace_actually_load_handler()がcallされる */
	}
